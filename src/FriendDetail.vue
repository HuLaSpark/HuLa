...
const extractRouteParam = (field: string): string | undefined => {
  const paramValue = route.params[field]
...
const params = computed(() => ({
  uid: extractRouteParam('uid'),
  type: Number(extractRouteParam('type'))
}))
...
const request = new HttpControl((signal, id: string) => apis.groupDetail({ id }, signal))

const setupGroup = (id: string) => {
  request.fetch(id).then((response) => {
    item.value = response
...
watchEffect(() => {
  const { type, uid } = params.value
  if (!uid) return

  if (type === RoomTypeEnum.SINGLE) {
    item.value = useUserInfo(uid).value
  } else {
    setupGroup(uid)
  }
})
...
<template>
  <main data-tauri-drag-region class="flex-1 bg-[--right-bg-color] h-full w-100vw min-w-600px">
    <!-- <Details data-tauri-drag-region :content="params" /> -->
    <!-- 好友详情 -->
    <n-flex v-if="params.type === RoomTypeEnum.SINGLE" vertical align="center" :size="30" class="mt-60px">
      <n-image
        object-fit="cover"
    ...
    <!-- 群聊详情 -->
    <div
      v-else-if="params.type === RoomTypeEnum.GROUP && item"
      ...
      <n-flex align="center" justify="space-between" class="px-30px box-border">
        ...
        <n-icon-wrapper
          @click="footerOptions[0].click(params.type)"
          class="cursor-pointer"
          :size="40"
          :border-radius="10"
          :color="'#13987f'">
