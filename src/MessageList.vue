...
const onMessageClick = (item: SessionItem) => {
  router.push({ path: `/home/message/room/${item.roomId}`, replace: true })
  handleMsgClick(item)
}
...
//   // 检查如果已经选中了一个会话，那么不再默认选中
// console.log('globalStore.currentSession ===>>', globalStore.currentSession);

//   const [ firstSession ] = sessionList.value ?? []
//   if (!firstSession) return
//   onMessageClick(firstSession)
// }

// 如果消息列表发生改变，那么应该选中第一个
// watch(sessionList, () => nextTick(handleSessionListChange))
...
onMounted(() => {
  ...
  // 如果没有打开 message，那么默认打开选中项
  console.log(route)

  if (route.name === 'message') {
    router.push({ path: `/home/message/room/${globalStore.currentSession.roomId}`, replace: true })
  }
})
</script>
